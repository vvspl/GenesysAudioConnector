...
import path from "path"; // Для збереження в файл
...
export class Server {
  ...
  start() {
    ...
    this.app = express();
    // Це дозволить відкривати файли з папки public/logs за посиланням /logs/ ім'я_файлу
    this.app.use(
      "/logs",
      express.static(path.join(__dirname, "../../public/logs")),
    );
    // -----------------------------------------------------------------
      ...
      (request: Request, socket: any, head: any) => {
        console.log(
          new Date().toISOString() +
            `:Received a connection request from ${request.url}.`,
        );
        ...
        verifyRequestSignature(request, this.secretService).then(
          (verifyResult) => {
            ...
            if (
              ...
              // <= For now always return VERIFIED for GOODKEY
              console.log(
                new Date().toISOString() +
                  ":" +
    ...
    this.wsServer.on("connection", (ws: WebSocket, request: Request) => {
      ...
      ws.on("error", (error: Error) => {
        ...
        console.log(
          new Date().toISOString() + ":" + `WebSocket Error: ${error}`,
        );
        ws.close();
      });
      ...
      ws.on("message", (data: WS.RawData, isBinary: boolean) => {
        ...
        if (ws.readyState !== WebSocket.OPEN) {
          console.log(
            new Date().toISOString() + ":" + `ws.on.message:!Websocket.OPEN`,
          );
          return;
        }
        const session = this.sessionMap.get(ws);
        if (!session) {
          ...
          console.log(
            new Date().toISOString() + ":" + "Session does not exist.",
          );
        ...
        if (isBinary) {
          ...
          console.log(
            new Date().toISOString() +
              ":" +
              `ws.on.message.notBinary.processTextMessageMessage|[${data.toString()}]`,
          );
