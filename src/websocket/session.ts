...
export class Session {
    ...
    getIsAudioPlaying() { return this.isAudioPlaying; }

    close() {
        if (this.closed) {
            console.log(new Date().toISOString()+':'+`[Session] WebSocket connection already Closed`);
            return;
        }
        try {
            ...
            console.log(new Date().toISOString()+':'+`[Session] Error while closing WebSocket: ${e}`);
        }
        this.closed = true;
        console.log(new Date().toISOString()+':'+`[Session] Session marked as closed.`);
    }

    setConversationId(conversationId: string) {
        this.conversationId = conversationId;
        console.log(new Date().toISOString()+':'+`[Session] Conversation ID set: ${conversationId}`);
    }

    setInputVariables(inputVariables: JsonStringMap) { 
        this.inputVariables = inputVariables; 
        console.log(new Date().toISOString()+':'+`[Session] Input variables set: ${JSON.stringify(inputVariables)}`);
    }
    getInputVariables() { return this.inputVariables; }

    setSelectedMedia(selectedMedia: MediaParameter) { 
        this.selectedMedia = selectedMedia; 
        console.log(new Date().toISOString()+':'+`[Session] Selected media set: ${JSON.stringify(selectedMedia)}`);
    }
    ...
    setIsAudioPlaying(isAudioPlaying: boolean) { 
        ...
        console.log(new Date().toISOString()+':'+`[Session] isAudioPlaying set to: ${isAudioPlaying}`);
    }
    processTextMessage(data: string) {
        if (this.closed) {
            console.log(new Date().toISOString()+':'+`[Session] Ignoring text message: session is closed.`);
            return;
        }
        ...
        if (message.id !== this.clientSessionId) {
            console.log(new Date().toISOString()+':'+`Invalid Client Session ID: ${message.id}.`);
            this.sendDisconnect('error', 'Invalid ID specified.', {});
            return;
        }
        console.log(new Date().toISOString()+':'+`Received a message of type '${message.type}' with parameters: ${JSON.stringify(message.parameters)}`);
        const handler = this.messageHandlerRegistry.getHandler(message.type);

        if (!handler) {
            console.log(new Date().toISOString()+':'+`Cannot find a message handler for '${message.type}'.`);
            return;
        }
    ...
    send(message: ServerMessage) {
        if (message.type === 'event') {
            console.log(new Date().toISOString()+':'+`Sending an ${message.type} message: ${message.parameters.entities[0].type}.`);
        } else {
            console.log(new Date().toISOString()+':'+`Sending a ${message.type} message.`);
        }        
    ...
    flushBuffer() {
        ...
        if(totalLength<=0) {
            console.log(new Date().toISOString()+':'+`[Session] flushBuffer called but buffer is empty.`);
            return;
        }
        ...
        if (bytes.length <= this.MAXIMUM_BINARY_MESSAGE_SIZE) {
            console.log(new Date().toISOString()+':'+`[Session] Sending ${bytes.length} binary bytes in 1 message.`);
            ...
            while (currentPosition < bytes.length) {
                ...
                console.log(new Date().toISOString()+':'+`[Session] Sending ${sendBytes.length} binary bytes in chunked message.`);
    ...
    sendAudioLegacy(currBytes: Uint8Array) {
        ...
        if(totalLength < this.MIN_BINARY_MESSAGE_SIZE) {
            console.log(new Date().toISOString()+':'+`[Session] Buffer size (${totalLength}) less than minimum, delaying send.`);
    ...
    sendTurnResponse(disposition: BotTurnDisposition, text: string | undefined, confidence: number | undefined) {
        ...
        console.log(new Date().toISOString()+':'+`[Session] Sending bot turn response: disposition=${disposition}, text=${text}, confidence=${confidence}`);
        this.send(message);
    }
    ...
    sendTranscript(transcript: string, confidence: number, isFinal: boolean) {
        ...
        if (channel) {
            ...
            console.log(new Date().toISOString()+':'+`[Session] Sending transcript: ${transcript}, confidence=${confidence}, isFinal=${isFinal}`);
            this.send(message);
        }
    }
    ...
    sendDisconnect(reason: DisconnectReason, info: string, outputVariables: JsonStringMap) {
        ...
        console.log(new Date().toISOString()+':'+`[Session] Sending disconnect: reason=${reason}, info=${info}`);
    ...
    sendClosed() {
        ...
        console.log(new Date().toISOString()+':'+`[Session] Sending closed message.`);
        this.send(message);
    }
    sendKeepAlive() {
        console.log(new Date().toISOString()+':'+`[Session] Sending keep-alive (pong) message.`);
    ...
    */
    processBinaryMessage(data: Uint8Array) {
        if (this.disconnecting || this.closed ) {
            console.log(new Date().toISOString()+':'+`[Session] Ignoring binary message: session disconnecting or closed.`);
            return;
        }
        // Ignore audio if we are capturing DTMF
        if (this.isCapturingDTMF) {
            console.log(new Date().toISOString()+':'+`[Session] Ignoring binary message: capturing DTMF.`);
            return;
        }       
        console.log(new Date().toISOString()+':'+`[Session] Processing binary audio message of length ${data.length}.`);
    ...
    */
    processDTMF(digit: string) {
        if (this.disconnecting || this.closed) {
            console.log(new Date().toISOString()+':'+`[Session] Ignoring DTMF: session disconnecting or closed.`);
            return;
        }
        /*
        ...
        */
        if (this.isAudioPlaying) {
            console.log(new Date().toISOString()+':'+`[Session] Ignoring DTMF: audio is playing.`);
        ...
        if (!this.isCapturingDTMF) {
            ...
            console.log(new Date().toISOString()+':'+`[Session] Started capturing DTMF.`);
        }
        if (!this.dtmfService || this.dtmfService.getState() === 'Complete') {
            this.dtmfService = new DTMFService()
                ...
                .on('final-digits', (digits) => {
                    ...
                    console.log(new Date().toISOString()+':'+`Captured DTMF Digits: ${digits}|To Be Implemented further`);
                    this.isCapturingDTMF = false;
                });
            console.log(new Date().toISOString()+':'+`[Session] DTMFService initialized.`);
        }
        console.log(new Date().toISOString()+':'+`[Session] Processing DTMF digit: ${digit}`);
