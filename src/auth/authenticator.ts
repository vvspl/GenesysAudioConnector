...
async function verifyRequestSignatureImpl(request: Request, secretService: SecretService): Promise<VerifyResult> {
    ...
    const result = await verifySignature({
        ...
        derivedComponentLookup: (name) => {
            if (name === '@request-target') {
                // ← Here we prepend the method
                //return `${request.method.toLowerCase()} ${request.url} HTTP/${request.httpVersion}`;
                return `${request.url}`;
            }
            return null;
        },
    ...
    console.log(new Date().toISOString() + `:Signature verification result for ${request.url}: ${JSON.stringify(result.code)}`);
