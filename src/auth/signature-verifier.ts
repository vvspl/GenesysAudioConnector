...
export const verifySignature = async (options: VerifierOptions): Promise<VerifyResult> => {
    ...
    console.debug('[Signature Debug] Base String:' + signatureData);
    ...
    const computedB64 = Buffer.from(computedSignature).toString('base64');
    const receivedB64 = Buffer.from(signature).toString('base64');
    console.debug('[Signature Debug] Computed Signature:', computedB64);
    console.debug('[Signature Debug] Received Signature:', receivedB64);
    ...
    if (timingSafeEqual(signature, computedSignature)) {
        if(badAlg) {
            console.debug('[Signature Debug] Signature algorithm is not supported:', alg);
            return badAlg;
        } else if(resolverResult.code === 'GOODKEY') { 
            console.debug('[Signature Debug] Signature verified successfully',resolverResult);
        ...
        if(badAlg) {
            ...
            console.debug('[Signature Debug] Signature does not match for a bad key:', resolverResult);
            return withFailure('FAILED', 'Signatures do not match');
        } else {
            console.debug('[Signature Debug] Signature does not match:', resolverResult);
            return withFailure('FAILED', 'Signatures do not match');
        }
    }
};
